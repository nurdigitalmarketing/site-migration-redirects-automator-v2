...
def setup_streamlit_interface():
    ...
    """
    st.set_page_config(page_title="Site Migration Redirects Automator v.2 | NUR® Digital Marketing", layout="wide")
    ...
    with col2:
        st.title('Site Migration Redirects Automator v.2')
...
def show_instructions_expander():
    ...
    """
    instructions = (
        "- Esegui la scansione del sito live e staging utilizzando Screaming Frog SEO Spider.\n"
        "- Esporta l'HTML come file CSV.\n"
        "- Carica i file CSV del sito 'Live' e 'Staging' utilizzando i caricamenti qui sotto.\n"
        "- Di default l'app cerca colonne chiamate 'Address', 'H1-1' e 'Title 1' ma possono essere mappate manualmente se non trovate.\n"
        "- Seleziona fino a 3 colonne che vuoi abbinare.\n"
        "- Clicca sul pulsante 'Processa File' per avviare il processo di abbinamento.\n"
...
def process_and_validate_uploaded_files(file_live, file_staging):
    ...
    if validate_uploaded_files(file_live, file_staging):
        ...
        if df_live.empty or df_staging.empty:
            show_warning_message("Attenzione: Uno o entrambi i file caricati sono vuoti.")
            return None, None
        else:
...
def process_uploaded_files_and_match_data(df_live, df_staging, matching_columns, progress_bar, message_placeholder,
    ...
    message_placeholder.success('Completato!')
